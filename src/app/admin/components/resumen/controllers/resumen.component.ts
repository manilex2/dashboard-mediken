...
export class ResumenComponent implements OnInit {
  ...
  @ViewChild("embedReportBtn") private embedBtnRef!: ElementRef<
    HTMLButtonElement
  >;
  ...
  async embedReport(): Promise<void> {
    ...
    try {
      ...
      this.displayMessage = `Failed to fetch config for report. ${error}`;
    ...
    const displayMessage = this.element.nativeElement.querySelector(
      ".display-message"
    );
    if (displayMessage) {
